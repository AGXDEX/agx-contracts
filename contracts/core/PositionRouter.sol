...
contract PositionRouter is BasePositionManager, IPositionRouter {
    ...
    function executeIncreasePositions(
        ...
        while (index < _endIndex) {
            ...
            (bool executeSuccess, bytes memory executeReturnData) = address(
                this
            ).call(
                    abi.encodeWithSignature(
                        "executeIncreasePosition(bytes32,address)",
                        key,
                        _executionFeeReceiver
                    )
                );
            if (!executeSuccess) {
                (bool cancelSuccess, bytes memory cancelReturnData) = address(
                    this
                ).call(
                        abi.encodeWithSignature(
                            "cancelIncreasePosition(bytes32,address)",
                            key,
                            _executionFeeReceiver
                        )
                    );
                if (cancelSuccess) {
                    bool _wasCancelled = abi.decode(cancelReturnData, (bool));
                    if (!_wasCancelled) {
                        break;
                    }
                }
            } else {
                bool _wasExecuted = abi.decode(executeReturnData, (bool));
                if (!_wasExecuted) {
                    break;
                }
            }
        ...
        while (index < _endIndex) {
            ...
            (bool executeSuccess, bytes memory executeReturnData) = address(
                this
                    ...
                    abi.encodeWithSignature(
                        "executeDecreasePosition(bytes32,address)",
                        key,
                        _executionFeeReceiver
                    )
                );
            if (!executeSuccess) {
                (bool cancelSuccess, bytes memory cancelReturnData) = address(
                    this
                        ...
                        abi.encodeWithSignature(
                            "cancelDecreasePosition(bytes32,address)",
                            key,
                            _executionFeeReceiver
                        )
                    );
    ...
    function _setTraderReferralCode(bytes32 _referralCode) internal {
        ...
        if (_referralStorage.traderReferralCodes(msg.sender) != bytes32(0)) {
            return;
        }
